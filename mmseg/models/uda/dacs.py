...
class DACS(UDADecorator):
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        # ]
        # grad_mag_before = calc_grad_magnitude(seg_grads)

        torch.nn.utils.clip_grad_norm_(parameters=self.get_model().parameters(), max_norm=5, norm_type=2)
        ...
        #     if p.grad is not None
        # ]
        # grad_mag_after = calc_grad_magnitude(seg_grads)

        # print(f'jyxjyxjyx step!!!! before={grad_mag_before}, after={grad_mag_after}')
    ...
    def masked_feat_dist(self, f1, f2, mask=None):
        ...
        # print('jyxjyxjyx masked_feat_dist:', mask is not None, pw_feat_dist[mask.squeeze(1)].shape, torch.mean(pw_feat_dist[mask.squeeze(1)]), pw_feat_dist[mask.squeeze(1)])
        if mask is not None:
    ...
    def calc_feat_dist(self, img, gt, feat=None):
        ...
        if self.fdist_classes is not None:
            ...
            # print('jyxjyxjyx lay:', lay)
            # print('jyxjyxjyx feat:', feat[lay].shape, torch.any(torch.isnan(feat[lay])).item(), torch.all(torch.isnan(feat[lay])).item(), feat[lay])
            # print('jyxjyxjyx feat_imnet:', feat_imnet[lay].shape, torch.any(torch.isnan(feat_imnet[lay])).item(), torch.all(torch.isnan(feat_imnet[lay])).item(), feat_imnet[lay])
            feat_dist = self.masked_feat_dist(feat[lay], feat_imnet[lay],
                                                fdist_mask)
            # print('jyxjyxjyx fdist_mask:', fdist_mask.float().sum())
            # print('jyxjyxjyx feat_dist:', feat_dist.shape, torch.any(torch.isnan(feat_dist)).item(), torch.all(torch.isnan(feat_dist)).item())
            self.debug_fdist_mask = fdist_mask
            self.debug_gt_rescale = gt_rescaled
        else:
            feat_dist = self.masked_feat_dist(feat[lay], feat_imnet[lay])
        # print('jyxjyxjyxjyx fdist_classes:', self.fdist_classes)
    ...
    def forward_train(self,
        ...
        # print('jyxjyxjyx loss', clean_log_vars, clean_loss)
        ...
        if not self.share_src_backward:
            ...
            if self.print_grad_magnitude:
                ...
                mmcv.print_log(f'Agg. Net. Grad.: {calc_grad_magnitude(self.get_model().backbone.dift_model.aggregation_network.mixing_weights_stride[0].grad)}')
                mmcv.print_log(f'Finecoder. conv1 Grad.: {calc_grad_magnitude(self.get_model().backbone.dift_model.finecoder.fusion_net.fusion1.resblock.conv1.weight.grad)}')
